...
class AdvancedNet2(Net2):
    ...
    def create_ssl_connection(self, hostname, port, timeout, **kwargs):
        ...
        for i in range(kwargs.get('max_retry', 4)):
            ...
            good_iplist_string = ""
	    for ip in good_ipaddrs:
                if good_iplist_string and not good_iplist_string.isspace():
                    good_iplist_string = (good_iplist_string + "|" + ip[0])
                else:
                    good_iplist_string = ip[0]
            logging.info('Good IP List: %s End of IP List', good_iplist_string)
